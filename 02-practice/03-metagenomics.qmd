...
Çalışma kapsamında kulanacağımız R kodlarını çalıştırmak için öncelikle `conda` ile R paketlerinin olduğu çevreyi kuralım. Ancak `Metaphlan3` programı bu paket içerisinde yok. O programı siz sunucu üzerinde kurmalısınız:
...
İlk olarak örneklerde bulunan Phylum içeriklerine bakalım. Bunun figürü oluşturmak için aşağıdaki komutu çalıştırmalısınız:
...
```bash
Rscript workflow/scripts/distance_plots.R --profile Data/microbial-profile.txt --metadata Data/microbial-metadata.txt --out1 results/BetaDiversity/Bray_Curtis.pdf --out2 results/esults/BetaDiversity/Figure-S3.pdf
```

Nmds figürü. Bu uzaklıkların projecte edlmiş halleri 
